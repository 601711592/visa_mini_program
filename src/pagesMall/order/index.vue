<template>
  <MallPage>
    <template v-if="order">
      <view class="mx-hero order-hero"><view class="order-hero__icon"><MallIcon :name="order.status === '已完成' ? 'check' : order.status === '待付款' ? 'clock' : 'order'" :size="58" /></view><view class="mx-title">{{ order.status === '售后退款' ? '退款审核中' : order.status }}</view><view class="mx-muted">{{ statusCopy }}</view></view>
      <view class="mx-body">
        <view v-for="line in order.lines" :key="line.id" class="mx-card"><ProductLine :line="line" variant="order" /><template v-if="line.travellersRequired"><view class="mx-line" /><view class="mx-subtitle">本商品出行人 · {{ line.travellers.length }} 位</view><view v-for="(person, index) in line.travellers" :key="index" class="mx-row"><view><view>{{ person.name }}</view><view class="mx-muted">生日 {{ person.birthday }}</view></view><text class="mx-muted">护照尾号 {{ person.passport.slice(-4) }}</text></view></template></view>
...
<style scoped>
.order-hero__icon { margin-bottom: 12rpx; }
.order-hero .mx-muted { margin: 12rpx 0 26rpx; font-size: 25rpx; }
</style>
