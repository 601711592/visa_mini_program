  ...
  <layout :navbar="{ backgroundColor: '#fff', fixed: true }">
    <view class="visa-condition-page">
      ...
      <view v-else-if="error" class="error-container">
        ...
        <button class="retry-button" @click="loadVisaData(visaTypeId)">重新加载</button>
      </view>
      ...
      <template v-else-if="conditionOptions.length > 0">
        ...
        <!-- 综合说明 -->
        <ComprehensiveNotes
          v-if="comprehensiveNotes.description !== '暂无综合说明'"
...
import type { VisaCondition, ApiResponse } from './constants.js';
...
const error = ref('');
const visaTypeId = ref('');
const conditionOptions = ref<VisaCondition[]>([]);
const comprehensiveNotes = ref({
  description: '暂无综合说明',
  imageUrl: '',
  videoUrl: '',
});
...
// 从接口加载数据
const loadVisaData = async (visaTypeId: string) => {
  if (!visaTypeId) {
  ...
  try {
    ...
    const response = await getVisaApplicationMaterialsByTypeId(visaTypeId);
    ...
    comprehensiveNotes.value = transformedData.comprehensiveNotes;
