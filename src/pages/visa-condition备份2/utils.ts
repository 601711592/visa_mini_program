 ...
 * 将API材料选项转换为组件材料选项
 */
function transformMaterialOption(apiOption: ApiMaterialOption, index: number): MaterialOption {
  return {
...
function transformOptionalMaterial(apiMaterial: ApiMaterial, index: number): OptionalMaterialGroup {
  const options = apiMaterial.options?.map((option, optionIndex) =>
    transformMaterialOption(option, optionIndex)
  ) || [];
...
export function transformApiDataToVisaConditions(apiData: ApiVisaCondition[]): {
  ...
  apiData.forEach((apiCondition, conditionIndex) => {
    ...
    // 处理综合说明（取第一个有说明的条件）
    if (apiCondition.descriptions && apiCondition.descriptions.length > 0 && comprehensiveNotes.description === '暂无综合说明') {
      const firstDesc = apiCondition.descriptions[0];
      comprehensiveNotes = {
        description: firstDesc.content,
        imageUrl: firstDesc.image_url || '',
        videoUrl: firstDesc.video_url || '',
      };
    }
  });
